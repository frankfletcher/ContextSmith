...
| **`evaluation_dimensions`** | `small_model_atomicity`, `instruction_clarity`, `output_contract_quality`, `context_strategy`, `assumption_control`, `domain_fit`, `validation_strength`, `loop_safety`, `git_file_safety`, `bloat_cognitive_load`, `progressive_disclosure`, `testability` | all enabled for reusable prompts | A-F quality eval rubrics |
...
| **`prompt_package_sections`** | `engineering_metadata`, `target_model_profile`, `context_strategy`, `interaction_mode`, `system_prompt`, `examples`, `loop_git_safety`, `persistent_task_state`, `subagent_delegation`, `runtime_recommendations`, `educational_change_report` | all included | Prompt sections |
