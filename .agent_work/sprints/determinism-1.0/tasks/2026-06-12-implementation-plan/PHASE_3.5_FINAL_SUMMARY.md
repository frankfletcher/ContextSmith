...
1. **Environment**: Use uv for all Python dependency management
2. **Commands**: All Python commands use `uv run`
3. **Linting**: All code must pass `uv run ruff check --select E,F,W,I`
4. **Formatting**: All code must pass `uv run ruff format --check`
5. **Testing**: All code must have tests, run with `uv run pytest`
6. **Dependencies**: Add with `uv add`, sync with `uv sync`
