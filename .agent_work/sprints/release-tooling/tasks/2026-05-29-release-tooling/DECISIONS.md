...
| D22 | test_release.sh captures `unzip -l` output to variable before grep | `set -euo pipefail` causes `unzip -l` piped to `grep -q` to fail intermittently due to pipe exit code handling — capturing to variable avoids the pipe entirely | More reliable zip content verification under strict shell settings |
