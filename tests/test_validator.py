...
class TestValidateRequirementsChain:
    ...
    def test_missing_id(self):
        result = validate_requirements_chain(_fixture("requirements_chain_missing_id.json"))
        assert result["passed"] is False
        assert any("Missing required field: id" in v for v in result["violations"])

    def test_invalid_status(self):
        result = validate_requirements_chain(_fixture("requirements_chain_invalid_status.json"))
        assert result["passed"] is False
        assert any("Invalid status" in v for v in result["violations"])

    def test_passed_without_evidence(self):
        result = validate_requirements_chain(_fixture("requirements_chain_passed_no_evidence.json"))
        assert result["passed"] is False
        assert any("requires at least one" in v for v in result["violations"])

    def test_empty_source(self):
        result = validate_requirements_chain(_fixture("requirements_chain_empty_source.json"))
        assert result["passed"] is False
        assert any("source" in v and "non-empty" in v for v in result["violations"])

    def test_wrong_artifact_type(self):
        result = validate_requirements_chain(_fixture("requirements_chain_wrong_artifact_type.json"))
        assert result["passed"] is False
        assert any("Wrong artifact_type" in v for v in result["violations"])

    def test_invalid_side_effect_tier(self):
        result = validate_requirements_chain(_fixture("requirements_chain_invalid_side_effect_tier.json"))
        assert result["passed"] is False
        assert any("Invalid side_effect_tier" in v for v in result["violations"])

    def test_invalid_validation_method(self):
        result = validate_requirements_chain(_fixture("requirements_chain_invalid_validation_method.json"))
        assert result["passed"] is False
...
class TestValidatePhaseContract:
    ...
    def test_wrong_artifact_type(self):
        result = validate_phase_contract(_fixture("phase_contract_wrong_artifact_type.json"))
        assert result["passed"] is False
        assert any("Wrong artifact_type" in v for v in result["violations"])
...
class TestValidateEvidenceLedger:
    ...
    def test_missing_requirement_id(self):
        result = validate_evidence_ledger(_fixture("evidence_ledger_missing_requirement_id.json"))
        assert result["passed"] is False
        assert any("Missing required field: requirement_id" in v for v in result["violations"])
    ...
    def test_wrong_artifact_type(self):
        result = validate_evidence_ledger(_fixture("evidence_ledger_wrong_artifact_type.json"))
        assert result["passed"] is False
        assert any("Wrong artifact_type" in v for v in result["violations"])

    def test_nonbool_verified(self):
        result = validate_evidence_ledger(_fixture("evidence_ledger_nonbool_verified.json"))
        assert result["passed"] is False
...
class TestValidateApprovalRecord:
    ...
    def test_missing_requirement_ids(self):
        result = validate_approval_record(_fixture("approval_record_missing_requirement_ids.json"))
        assert result["passed"] is False
        assert any("Missing required field: requirement_ids" in v for v in result["violations"])

    def test_approved_without_approver(self):
        result = validate_approval_record(_fixture("approval_record_approved_no_approver.json"))
        assert result["passed"] is False
    ...
    def test_wrong_artifact_type(self):
        result = validate_approval_record(_fixture("approval_record_wrong_artifact_type.json"))
        assert result["passed"] is False
        assert any("Wrong artifact_type" in v for v in result["violations"])

    def test_requested_bad_approver(self):
        """Status 'requested' requires approver='not_yet_approved' and timestamp='pending'."""
        result = validate_approval_record(_fixture("approval_record_requested_bad_state.json"))
        assert result["passed"] is False
        assert any("requested" in v and "not_yet_approved" in v for v in result["violations"])
    ...
    def test_denied_without_approver(self):
        ...
        result = validate_approval_record(_fixture("approval_record_denied_no_approver.json"))
        assert result["passed"] is False
        assert any("requires an approver" in v for v in result["violations"])
        assert any("requires a timestamp" in v for v in result["violations"])
    ...
    def test_highrisk_no_residual_risk(self):
        ...
        result = validate_approval_record(_fixture("approval_record_highrisk_no_risk.json"))
        assert result["passed"] is False
        assert any("residual_risk" in v and "non-empty" in v for v in result["violations"])
...
class TestValidatePhaseCloseout:
    ...
    def test_missing_phase_id(self):
        result = validate_phase_closeout(_fixture("phase_closeout_missing_phase_id.json"))
        assert result["passed"] is False
        assert any("Missing required field: phase_id" in v for v in result["violations"])
    ...
    def test_wrong_artifact_type(self):
        result = validate_phase_closeout(_fixture("phase_closeout_wrong_artifact_type.json"))
        assert result["passed"] is False
        assert any("Wrong artifact_type" in v for v in result["violations"])

    def test_nonbool_objective_met(self):
        result = validate_phase_closeout(_fixture("phase_closeout_nonbool_objective_met.json"))
        assert result["passed"] is False
        assert any("objective_met" in v and "boolean" in v for v in result["violations"])
...
class TestValidateDomainPack:
    ...
    def test_missing_approval_gates(self):
        ...
        assert result["passed"] is False
        assert any("Missing required field: approval_gates" in v for v in result["violations"])
    ...
    def test_missing_boundary_gate(self):
        ...
        result = validate_domain_pack(_fixture("domain_pack_missing_boundary_gate.json"))
        assert result["passed"] is False
        assert any("requires_approval" in v and "approval_gates" in v
    ...
    def test_invalid_domain_format(self):
        ...
        assert result["passed"] is False
        assert any("domain" in v.lower() and "lowercase" in v.lower()
                    for v in result["violations"])
    ...
    def test_invalid_boundary_value(self):
        ...
        result = validate_domain_pack(_fixture("domain_pack_invalid_boundary_value.json"))
        assert result["passed"] is False
        assert any("Invalid" in v and "external_action_boundaries" in v
                    for v in result["violations"])
    ...
    def test_empty_residual_risk(self):
        ...
        assert result["passed"] is False
        assert any("residual_risk" in v and "non-empty" in v
                    for v in result["violations"])
...
class TestCrossArtifact:
    ...
    def test_workflow_all_invalid(self):
        ...
        result = validate_workflow({
            ...
            "evidence_ledger": _fixture("evidence_ledger_missing_requirement_id.json"),
            "phase_closeout": _fixture("phase_closeout_missing_phase_id.json"),
        })
        assert result["passed"] is False
        assert any("requirements_chain: Missing required field: id" in v
                   for v in result["violations"])
        assert any("evidence_ledger: Missing required field: requirement_id" in v
                   for v in result["violations"])
        assert any("phase_closeout: Missing required field: phase_id" in v
                   for v in result["violations"])
...
class TestWarnings:
    def test_requirements_chain_pending_no_phases(self):
        ...
        assert result["passed"] is True
        assert any("no phases" in w.lower() or "still pending" in w.lower()
    ...
    def test_approval_record_empty_requirement_ids(self):
        ...
        assert result["passed"] is False
        assert any("requirement_ids" in v and "non-empty" in v
                   for v in result["violations"])
